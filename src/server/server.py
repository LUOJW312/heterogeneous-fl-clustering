...
class Server(object):
    ...
    def boot(self):
        ...
        # Add fl_model to import path  将 fl_model 添加到导入路径
        sys.path.append(model_path)

        # Set up simulated server    设置模拟服务器
        self.load_data()
        self.load_model()
    ...
    def load_data(self):
        ...
        # Extract config for loaders   加载程序的提取配置
        config = self.config

        # Set up data generator  设置数据生成器
    ...
    def load_model(self):
        ...
        # Set up global model  设置全局模型
        self.model = fl_model.Net()
        self.save_model(self.model, model_path)

        # Extract flattened weights (if applicable)  提取权重
    ...
    def make_clients(self, num_clients):
        ...
        # Make simulated clients  制作模拟客户端
        clients = []
        for client_id in range(num_clients):

            # Create new client  创造新的客户端
            new_client = client.Client(client_id)

            if not IID:  # Configure clients for non-IID data  为非iid数据配置客户端
                if self.config.data.bias:
                    # Bias data partitions 偏差数据分区
        ...
        if loading == 'static':
            ...
            # Send data partition to all clients   将数据分布输送到各个客户端
    ...
    def run(self):
        ...
        # Perform rounds of federated learning   执行多轮联合学习
        for round in range(1, rounds + 1):
            logging.info('**** Round {}/{} ****'.format(round, rounds))

            # Run the federated learning round  运行联合学习轮次
            accuracy = self.round()

            # Break loop when target accuracy is met  满足目标精度时中断循环
    ...
    def round(self):
        ...
        # Run clients using multithreading for better parallelism  使用多线程运行客户端以实现更好的并行度
        threads = [Thread(target=client.run) for client in sample_clients]
        [t.start() for t in threads]
        [t.join() for t in threads]

        # Recieve client updates  接收客户端更新
        reports = self.reporting(sample_clients)

        # Perform weight aggregation  执行权重聚合
        logging.info('Aggregating updates')
        updated_weights = self.aggregation(reports)

        # Load updated weights  上传更新的权重
        ...
        # Save updated global model  保留更新后的全局模型
        self.save_model(self.model, self.config.paths.model)

        # Test global model accuracy  测试全局模型的准确度
        if self.config.clients.do_test:  # Get average accuracy from client reports
            accuracy = self.accuracy_averaging(reports)
        else:  # Test updated model on server  在客户端上测试更新的模型
